...
START_TEST(s21_add_test_default) {
  printf("---------------------------------------------\n");
...
START_TEST(s21_add_test_minus_res) {
  s21_decimal d1 = {{123, 0, 0, 0}};
...
START_TEST(s21_add_test_small_float) {
  s21_decimal d1 = {{123, 0, 0, 0}};
...
START_TEST(s21_add_test_dif_exp) {
  s21_decimal d1 = {{123, 0, 0, 0}};
  s21_decimal d2 = {{4321, 0, 0, 0}};
  d1.bits[3] += 3 << 16;
  d2.bits[3] += 4 << 16;
...
START_TEST(s21_add_test_dif_exp2) {
  s21_decimal d1 = {{123, 0, 0, 0}};
  s21_decimal d2 = {{4321, 0, 0, 0}};
  d1.bits[3] += 4 << 16;
  d2.bits[3] += 3 << 16;
  s21_decimal result = {{0, 0, 0, 0}};
  s21_add(d1, d2, &result);
  float expected = 0.0123 + 4.321;
  float real;
  // s21_decimal_print(&d1);
  // s21_decimal_print(&d2);
  // s21_decimal_print(&result);
  s21_from_decimal_to_float(result, &real);
  printf("Input: 0.0123 + 4.321\nReal output: %f\nExpected: %f\n", real,
         expected);
  printf(real - expected < 1e-6 ? "SUCCESS" : "FAIL");
  printf("\n---------------------------------------------\n");
  ck_assert_float_eq_tol(real, expected, 1e-6);
...
START_TEST(s21_add_test_bignums) {
  s21_decimal d1 = {{0xFFFFFFFF, 0, 0, 0}};
  s21_decimal d2 = {{0xFFFFFFFF, 0, 0, 0}};
  s21_decimal result = {{0, 0, 0, 0}};
  s21_add(d1, d2, &result);
  long int expected = 0x1FFFFFFFE;
  long int real = result.bits[0] + ((long int)result.bits[1] << 32);
  printf("Input: 0xFFFFFFFF + 0xFFFFFFFF\nReal output: %ld\nExpected: %ld\n",
         real, expected);
  printf(real == expected ? "SUCCESS" : "FAIL");
  printf("\n---------------------------------------------\n");
  ck_assert_int_eq(real, expected);
...
START_TEST(s21_add_test_overfill_err) {
  s21_decimal d1 = {{0xFFFFFFFF, 0, 0xFFFFFFFF, 0}};
  s21_decimal d2 = {{0xFFFFFFFF, 0, 0xFFFFFFFF, 0}};
  s21_decimal result = {{0, 0, 0, 0}};
  int expected = 1;
  int real = s21_add(d1, d2, &result);
  // s21_decimal_print(&result);
  // long int real = result.bits[0] + ((long int)result.bits[1]<<32);
  printf(
...
START_TEST(s21_add_test_bank_roundingp1) {
  s21_decimal d1 = {{0xFFFFFFFF, 0, 0xFFFFFFFF, 0}};
  s21_decimal d2 = {{151234, 0, 0, 0}};
  s21_decimal result = {{0, 0, 0, 0}};
  d2.bits[3] += 5 << 16;
  unsigned int ex1 = 0x0;
  unsigned int ex2 = 0x1;
...
START_TEST(s21_add_test_bank_roundingp2) {
...
Suite *s21_add_suite(void) {
  ...
  tcase_add_test(tc_core, s21_add_test_default);
  tcase_add_test(tc_core, s21_add_test_minus);
  tcase_add_test(tc_core, s21_add_test_minus_res);
  tcase_add_test(tc_core, s21_add_test_small_float);
  tcase_add_test(tc_core, s21_add_test_bignums);
  tcase_add_test(tc_core, s21_add_test_dif_exp);
  tcase_add_test(tc_core, s21_add_test_dif_exp2);
  tcase_add_test(tc_core, s21_add_test_overfill_err);
  tcase_add_test(tc_core, s21_add_test_bank_roundingp1);
  tcase_add_test(tc_core, s21_add_test_bank_roundingp2);
